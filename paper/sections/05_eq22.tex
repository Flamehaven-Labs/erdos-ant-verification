...
The computation also forms $K\pi$ with
$K \approx 7.62 \cdot 10^{17}$ before dividing by $18r^3$ to obtain a
ratio extremely close to $1$. At ordinary double precision the
near-one ratio is rounded at a scale larger than the
$10^{-18}$ separation, so the cancellation happens before the logarithm:
the naive evaluation rounds the ratio to exactly $1$, yields
...
The $200$-bit precision is a conservative budget, not an unexplained
output target. Resolving a $1.31 \cdot 10^{-18}$ numerator separation
requires about $\log_2(1/(1.31 \cdot 10^{-18})) \approx 59.4$ bits just
to see the signal; keeping the relative error in that small numerator
below the $10^{-3}$ regression threshold adds about $10$ guard bits.
The chosen $200$ bits leave a large deterministic margin over this
rough $70$-bit lower bound while remaining fast enough for CI.
...
The observed relative error is $1.4 \cdot 10^{-4}$, i.e.\ $0.014\%$;
the threshold is $10^{-3}$, i.e.\ $0.1\%$.

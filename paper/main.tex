% main.tex
% Executable Reproduction of Equation (2.2)
% in the OpenAI Erdős Unit-Distance Disproof Remarks
%
...
\hypersetup{
  ...
  pdftitle={Executable Reproduction of Equation (2.2) in the OpenAI Erdős Unit-Distance Disproof Remarks},
...
\title{%
  Executable Reproduction of Equation (2.2)\\
  in the OpenAI Erd\H{o}s Unit-Distance\\
  Disproof Remarks\\[0.5em]
  \large A High-Precision Python Check of the Published Eq.~(2.2) Calculation}
\author{%
  Yun Kwansub%
  \thanks{Independent author. Code distribution:
    \href{https://github.com/Flamehaven-Labs/openai-erdos-eq22-reproduction}{github.com/Flamehaven-Labs/openai-erdos-eq22-reproduction}.}%
}
\date{Version 0.2.3 --- 2026-05-25}
...
\begin{abstract}
We present \texttt{openai-erdos-eq22-reproduction}, an open-source Python package
...
that equation states a rigorous exponent excess of approximately
$6.24 \times 10^{-38}$ above the classical Erd\H{o}s exponent. Our
\texttt{mpmath} evaluation
...
at \url{https://github.com/Flamehaven-Labs/openai-erdos-eq22-reproduction}.

\noindent\textbf{Reproduction.}
The repository provides a one-command verifier and frozen JSON/Markdown
evidence files; the exact commands and evidence paths are given in
\S\ref{sec:repro}. The first paper-bearing release corresponds to
commit \texttt{c8fe529} (release tag \texttt{v0.1.3}) with Zenodo DOI
\href{https://doi.org/10.5281/zenodo.20377950}{10.5281/zenodo.20377950}.\footnote{The earlier source-only release \texttt{v0.1.1} carries DOI \href{https://doi.org/10.5281/zenodo.20366884}{10.5281/zenodo.20366884}. The intermediate \texttt{v0.1.2} was a pre-PDF snapshot, and \texttt{v0.1.3}/\texttt{v0.1.4} bundled the first compiled paper. The tagged \texttt{v0.2.0} paper was a documentation-only revision that tightened scope wording and removed project-specific terminology. The \texttt{v0.2.1} source revision added verifier-process hardening, a source-code alignment audit, and paper/docs consistency corrections. The \texttt{v0.2.2} revision aligned the public repository slug, README display name, paper title, and metadata naming. This \texttt{v0.2.3} revision improves PDF readability, public-source framing, and precision rationale; the eq.~(2.2) numerical claim and test count are unchanged.}
\end{abstract}
